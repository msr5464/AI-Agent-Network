...
PLAYWRIGHT_HEADLESS used to be read by four steps with four copies of the same
...
def test_recognised_spellings(monkeypatch, raw, expected):
    monkeypatch.setenv("PLAYWRIGHT_HEADLESS", raw)
...
def test_mcp_config_follows_the_switch_when_no_caller_says_otherwise(tmp_path,
                                                                    monkeypatch):
    monkeypatch.setenv("PLAYWRIGHT_HEADLESS", "false")
    written = mcp_config.write_playwright_mcp_config(tmp_path)
...
def test_mcp_config_still_honours_an_explicit_argument(tmp_path, monkeypatch):
    monkeypatch.setenv("PLAYWRIGHT_HEADLESS", "true")
    written = mcp_config.write_playwright_mcp_config(tmp_path, headless=False)
    assert "--headless" not in written.read_text()
...
def test_attaching_over_cdp_never_argues_about_headless(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("PLAYWRIGHT_HEADLESS", "true")
    written = mcp_config.write_playwright_mcp_config(
