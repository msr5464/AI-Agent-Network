# qa-auto-fix — Master Context

Read this file first. Every time. Before doing anything else.

## What This Agent Does

Picks up automation issues queued by `qa-auto-analyse`, fixes broken locators via Claude,
...
```
agents/qa-auto-fix/queue/<build_tag>.json   ← written by qa-auto-analyse
    ↓
...
Written by `qa-auto-analyse/actions/05_ship.py`. Contains everything needed — no DB required.
...
```json
{
  ...
  "created_at": "...",
  "source_session": "...",
  "source_audit_dir": "/abs/path/to/qa-auto-analyse/audit/<session>",
  "automation_issues": [
    {
      "test_name": "...",
...
**Session folder:** `agents/qa-auto-fix/audit/$SESSION_ID/`
...
| `WORKSPACE_DIR` | Parent directory for the automation repo. Must be outside QA-AI-Agent. If the repo is not present, qa-auto-fix clones it automatically using `GITHUB_TOKEN` + `GITHUB_ORG` + `GITHUB_REPO_AUTOMATION`. |
...
| `REPO_CONTEXT_FILE` | Path to conventions file in the automation repo (relative to repo root or absolute). If unset or not found, falls back to `agents/qa-auto-fix/CONVENTIONS.md` bundled in this agent. |
...
make run AGENT=qa-auto-fix

# Direct mode — process a specific build tag
make run AGENT=qa-auto-fix BUILD_TAG=ProdSanity-All-Tests-541

# Dry-run — fixes applied and tested locally, but no PR pushed
AUTO_PUSH=false make run AGENT=qa-auto-fix

# View audit trail
make audit AGENT=qa-auto-fix
make audit AGENT=qa-auto-fix SESSION=20260328-143022-fix-ProdSanity-All-Tests-541
```
