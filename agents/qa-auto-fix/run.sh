...
#
# Usage (via Makefile):
#   make run AGENT=qa-auto-fix                           # queue mode: picks oldest
#   make run AGENT=qa-auto-fix BUILD_TAG=ProdSanity-541  # direct: specific handoff
#   AUTO_PUSH=false make run AGENT=qa-auto-fix           # dry-run: no PR
#
...
if [[ -n "$HANDOFF_FILE" ]]; then
  ...
  if [[ ! -f "$HANDOFF_FILE" ]]; then
    ...
    log "Run qa-auto-analyse first, or check the queue directory."
    exit 1
  fi
  MODE="direct"
  ...
  if [[ -z "$HANDOFF_FILE" ]]; then
    ...
    log "Run qa-auto-analyse first to populate the queue."
    exit 0
  fi
  BUILD_TAG=$(python3 -c "import json,sys; print(json.load(open(sys.argv[1]))['build_tag'])" "$HANDOFF_FILE")
  SAFE_TAG="${BUILD_TAG//\//-}"
