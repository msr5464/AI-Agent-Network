# qa-auto-analyse — Master Context

Read this file first. Every time. Before doing anything else.

## What This Agent Does

Autonomously analyses test build results, classifies failures as PRODUCT_BUG vs AUTOMATION_ISSUE, reviews classifications independently via an adversarial debate, generates an HTML report, and queues fixable automation issues for the `qa-auto-fix` agent.
...
| **05 Ship** | HTML report, write handoff.json to qa-auto-fix queue, Slack notify | No AI, no code changes, no PR |
...
05-ship.json             (report_path, handoff_path, slack_notified)
    ↓
agents/qa-auto-fix/queue/<build_tag>.json   ← consumed by qa-auto-fix
```
...
**Session folder:** `agents/qa-auto-analyse/audit/$SESSION_ID/`
...
| `AUTOFIX_QUEUE_DIR` | Path to qa-auto-fix queue dir (default: `agents/qa-auto-fix/queue` inside repo) |
...
make run AGENT=qa-auto-analyse

# Direct mode — analyze a specific build tag
make run AGENT=qa-auto-analyse BUILD_TAG=ProdSanity-All-Tests-541

# Stop at any step for inspection
STOP_AFTER=collect make run AGENT=qa-auto-analyse
STOP_AFTER=classify make run AGENT=qa-auto-analyse BUILD_TAG=ProdSanity-All-Tests-541

# View audit trail
make audit AGENT=qa-auto-analyse
make audit AGENT=qa-auto-analyse SESSION=20260328-143022-ProdSanity-All-Tests-541
```
