...
"""
Step 05 — Ship
Check .verdict gate, generate HTML report, write handoff.json for qa-auto-fix,
...
failures, writes agents/qa-auto-fix/queue/<build_tag>.json for the qa-auto-fix agent
...
# qa-auto-fix queue — handoff destination
# Override with AUTOFIX_QUEUE_DIR env var if qa-auto-fix lives elsewhere
_queue_dir_env = os.environ.get("AUTOFIX_QUEUE_DIR", "")
AUTOFIX_QUEUE_DIR = Path(_queue_dir_env) if _queue_dir_env else REPO_ROOT / "agents" / "qa-auto-fix" / "queue"
...
def send_slack(channel: str, text: str):
    if not SLACK_BOT_TOKEN or not channel:
        log("Slack not configured — skipping notification")
        return False
...
def write_handoff(build_tag: str, classify_data: dict, collect_data: dict) -> str | None:
    """
    Write handoff.json to qa-auto-fix queue if there are eligible candidates.
...
def generate_html_report(collect_data: dict, classify_data: dict) -> str | None:
    try:
        ...
        from src.parsers.models import TestResult, TestStatus, TestSummary
        ...
        from src.utils import FailureClassificationUtils, TestDataCache
...
def main():
    ...
    # ── Write handoff for qa-auto-fix (only if APPROVED) ─────────────────────
    if not escalated:
    ...
    if not escalated and SLACK_NOTIFY_CHANNEL:
        ...
        if handoff_path:
            ...
            lines.append(f":wrench: {eligible_count} automation issue(s) queued for qa-auto-fix")
        else:
    ...
    if handoff_path:
        md_lines += [
            ...
            "## Handoff",
            "",
            f"Queued for qa-auto-fix: `{handoff_path}`",
            "Run: `make run AGENT=qa-auto-fix`",
        ]
